...
CREATE OR REPLACE PACKAGE BODY PA_BIBLIOTECARIO IS 
    ...
    BEGIN
    AUT:= PC_LIBROS.CO_AUTOR(autorx);
    RETURN AUT;
    END;
    PROCEDURE reservarSalon (afiliadox varchar, numSalonx number, bibliotecax varchar, bibliotecariox varchar, iniciox DATE, finx DATE) IS 
    BEGIN
    ...
    BEGIN
    AF:= PCK_AFILIADO.BUSCAR_AFILIADO_CORREO(correox);
    RETURN AF;
    END;
    FUNCTION BUSCAR_AFILIADO_CEDULA(cedulax IN VARCHAR) RETURN SYS_REFCURSOR  IS AF SYS_REFCURSOR;
    BEGIN
    AF:= PCK_AFILIADO.BUSCAR_AFILIADO_CEDULA(cedulax);
    RETURN AF;
    END;
